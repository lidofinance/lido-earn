...
* `RecoveryNotActive`: Thrown when emergency recovery is not active.
...
* `RecoverableAmountMismatch`: Thrown when declared recoverable amount exceeds the vault's idle token balance.
...
**Constraints:**
  ...
  * Reverts with `InsufficientLiquidity` if the requested amount exceeds `TARGET_VAULT.maxWithdraw`.
